...
var DataRepo = (function () {
    ...
    DataRepo.prototype.applyDataDiffs = function (name, diff_) {
        ...
        if (Array.isArray(diff_)) {
            diff_.forEach(function (item) { return deepDiff.applyChange(synceddata.current, synceddata.current, item); });
        }
        else {
            deepDiff.applyChange(synceddata.current, synceddata.current, diff_);
        }
        return true;
    };
    return DataRepo;

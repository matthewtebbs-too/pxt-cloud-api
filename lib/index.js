...
var DataRepo = (function () {
    ...
    DataRepo.calcDataDiff = function (lhs, rhs, options) {
        var diff_ = deepDiff.diff(lhs, rhs, options ? options.filter : undefined);
        return diff_ ? diff_.map(function (d) { return DataRepo.encode(d); }) : [];
    };
    DataRepo.cloneData = function (current, options) {
        var objStack = [];
        var path = [];
        return lodash.cloneDeepWith(current, function (value, key, object) {
            var isObject = lodash.isObject(value);
            if (undefined !== key) {
                for (;;) {
                    var top_1 = lodash.last(objStack);

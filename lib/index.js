...
var DataRepo = (function () {
    ...
    DataRepo.encode = function (data, asArray) {
        return asArray ? data.map(function (d) { return msgpackLite.encode(d); }) : msgpackLite.encode(data);
    };
    DataRepo.decode = function (buffer) {
        return Array.isArray(buffer) ? buffer.map(function (b) { return msgpackLite.decode(b); }) : msgpackLite.decode(buffer);
    };
    ...
    DataRepo.filteredData = function (current, options) {
        return lodash.omitBy(current, function (value, key) {
